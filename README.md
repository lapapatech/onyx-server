...
**El CLI que usa el target NO está en este repo.** Está en `~/proyectos/onyx-qwen/` (fork de Qwen Code).

## Arquitectura

```
onyx-qwen (CLI bonito) ──HTTP──► Backend Onyx (este repo) ──HTTP──► DeepSeek API
                                      │
                                      ▼
                                   SQLite (logs)
```
